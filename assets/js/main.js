...
var productsListEndPoint = 'https://awesome-arcgis.maps.arcgis.com/sharing/rest/content/groups/663480a878724c42aef09a523a8d5139/search';
var params = {
    q: '-type:"Code Attachment" -type:"Featured Items" -type:"Symbol Set" -type:"Color Set" -type:"Windows Viewer Add In" -type:"Windows Viewer Configuration" -type:"Map Area" -typekeywords:"MapAreaPackage"',
    num: 100,
    start: 1,
    sortField: 'title',
    sortOrder: 'asc',
...
$.getJSON(productsListEndPoint, params, function(data){

    var template = $.templates('#productCard');

    data = data.results;
    console.log(`There are ${data.length} products in this list`);

    products = shuffle(data);
    var htmlOutput = template.render(products);

    $('#product-list').html(htmlOutput);
...
$.views.converters('getSlug', function(val) {
    var urlSplitted = val.split('/');

    return urlSplitted[urlSplitted.length-2];
});

$.views.converters('removeWikiPage', function(val) {
    if(val.indexOf(" - Extension (wiki page)") != -1){
        return val.split(" - Extension (wiki page)")[0];
    }
    return val.split("(wiki page)")[0];
});
...
$.views.helpers({
  utils: {
    isExtension: function(val) {
      return val.indexOf("Extension") !== -1;
    }
  }
})
